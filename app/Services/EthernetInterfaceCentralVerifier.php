...
class EthernetInterfaceCentralVerifier
{
    /**
     * @return array{
    ...
    public function verify(Task $task, CentralAPIHelper $helper): array
    {
        $interfaces = $task->deviceInterfaces()
            ->with(['device.interfaces.lacp_profile', 'switch_port', 'stp_profile'])
            ->get();
    ...
    public function verifyInterfaces(Collection $interfaces, CentralAPIHelper $helper): array
    {
        ...
        foreach ($interfaces->groupBy('device_id') as $deviceId => $deviceInterfaces) {
            ...
            if (! array_key_exists($device->id, $centralByDevice)) {
                $fetch = $this->fetchEthernetInterfacesByName($helper, $device);
     ...
     */
    protected function fetchEthernetInterfacesByName(CentralAPIHelper $helper, Device $device): array
    {
        $response = $helper->get_ethernet_interfaces($device);
    ...
    {
        return CentralAPIHelper::build_ethernet_interface_patch_body($deviceInterface);
    }
